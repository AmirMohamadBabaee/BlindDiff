...
class DiffusionModel(BaseModel):
    ...
    def feed_data(self, data):
        ...
        if "lq" in data and self.flag == False:
            ...
            self.lq, self.ker_code, self.ker, self.structure_gt, _ = self.prepro(
                self.gt, True
            )
    ...
    def test(self):
        ...
        if tile_size > 0:
            mod_pad_h, mod_pad_w = 0, 0
            self.output = torch.zeros(b, c, h * scale, w * scale)
            assert (
                tile_size % scale == 0
                ...
                for j in range(0, w-1, lq_tile_size):
                    ...
                    if hasattr(self, "net_g_ema"):
                        ...
                        tile_output = self.diffusion.p_sample_loop(
                            ...
                            progress=True,
                            device=self.device,
                        )  # + F.interpolate(model_kwargs["cond"], scale_factor=4, mode='bicubic')
                        # print(samples.shape)
                        # input()
                        # tile_output= self.net_g_ema(self.lq)
